...
type sessionPayload struct { Username string `json:"username"`; Expires int64 `json:"expires"`; Nonce string `json:"nonce"` }
type auditEvent struct { Time string `json:"time"`; Type string `json:"type"`; Message string `json:"message"`; Remote string `json:"remote"` }
type serviceStatus struct { Name string `json:"name"`; Port int `json:"port"`; Online bool `json:"online"`; LatencyMS int64 `json:"latency_ms"`; Error string `json:"error,omitempty"` }

var username, password, nvrHost string
var secret []byte
...
func verifyToken(token string) (*sessionPayload, bool) {
    ...
    if subtle.ConstantTimeCompare([]byte(expected), []byte(parts[1])) != 1 { return nil, false }
    raw, err := base64.RawURLEncoding.DecodeString(parts[0])
    if err != nil { return nil, false }
    var payload sessionPayload
    if json.Unmarshal(raw, &payload) != nil || payload.Expires < time.Now().Unix() || payload.Username != username { return nil, false }
    return &payload, true
}
...
func requireSession(next http.HandlerFunc) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        if _, ok := sessionFromRequest(r); !ok { jsonResponse(w, http.StatusUnauthorized, map[string]bool{"authenticated":false}); return }
        next(w, r)
    }
}
...
func addEvent(kind, message, remote string) {
    ...
    if len(events) > 100 { events = events[:100] }
}
...
func probe(name string, port int) serviceStatus {
    ...
    if err != nil { status.Error = err.Error(); return status }
    status.Online = true
    _ = conn.Close()
    return status
}
...
func main() {
    ...
    if nvrHost == "" { nvrHost = "10.10.1.2" }
    if password == "" || len(secret) < 32 { log.Fatal("APP_PASSWORD and SESSION_SECRET (32+ characters) are required") }
    seconds, _ := strconv.Atoi(os.Getenv("SESSION_TTL_SECONDS")); if seconds <= 0 { seconds = 43200 }
    ttl = time.Duration(seconds) * time.Second
    addEvent("system", "CCTV Dashboard API started", "local")

    mux := http.NewServeMux()
    mux.HandleFunc("/api/login", func(w http.ResponseWriter, r *http.Request) {
        if r.Method != http.MethodPost { jsonResponse(w, 405, map[string]string{"error":"method not allowed"}); return }
        var req loginRequest
        dec := json.NewDecoder(http.MaxBytesReader(w, r.Body, 4096))
        if dec.Decode(&req) != nil { jsonResponse(w, 400, map[string]string{"error":"invalid JSON"}); return }
        ...
        if !userOK || !passOK { time.Sleep(250*time.Millisecond); addEvent("security", "Failed login attempt", r.RemoteAddr); jsonResponse(w, 401, map[string]string{"error":"invalid username or password"}); return }
        token, err := createToken(username); if err != nil { jsonResponse(w, 500, map[string]string{"error":"could not create session"}); return }
        maxAge := 0
        if req.Remember { maxAge = int(ttl.Seconds()) }
    ...
    mux.HandleFunc("/api/session", func(w http.ResponseWriter, r *http.Request) {
        ...
        if !ok { jsonResponse(w, 401, map[string]bool{"authenticated":false}); return }
    ...
    mux.HandleFunc("/auth/check", func(w http.ResponseWriter, r *http.Request) {
        if _, ok := sessionFromRequest(r); !ok { w.WriteHeader(http.StatusUnauthorized); return }
        w.WriteHeader(http.StatusNoContent)
    })
    mux.HandleFunc("/api/system/status", requireSession(func(w http.ResponseWriter, r *http.Request) {
        ports := []struct{name string; port int}{{"RTSP",554},{"Web service",8181},{"Device service",23000},{"XMeye protocol",34567}}
        results := make([]serviceStatus, len(ports))
        var wg sync.WaitGroup
        for i, p := range ports { wg.Add(1); go func(i int, p struct{name string; port int}) { defer wg.Done(); results[i] = probe(p.name,p.port) }(i,p) }
        wg.Wait()
        online := 0; for _, s := range results { if s.Online { online++ } }
    ...
    mux.HandleFunc("/api/events", requireSession(func(w http.ResponseWriter, r *http.Request) {
        eventsMu.Lock(); snapshot := append([]auditEvent(nil), events...); eventsMu.Unlock()
    ...
    server := &http.Server{Addr:":3000", Handler:mux, ReadHeaderTimeout:5*time.Second, ReadTimeout:10*time.Second, WriteTimeout:10*time.Second, IdleTimeout:60*time.Second}
